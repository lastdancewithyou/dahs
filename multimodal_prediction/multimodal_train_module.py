...
def train_multimodal_model(ts_df, img_df, text_df, lambda_ce=0.3, patience=5):
    ...
    train_loader, val_loader, test_loader = get_dataloaders(ts_df, img_df, text_df, batch_size=18, train_ratio=0.7, val_ratio=0.2, random_seed=0)
    ...
    for epoch in tqdm(range(num_epochs), total=num_epochs, desc="<Training Processing>"):
        ...
        for batch in train_loader: 
            ...
            contrastive_loss = total_contrastive_loss(ts_embeddings, cxr_embeddings, text_embeddings, fused_embeddings, labels, supcon_loss_fn, lambda_intra, lambda_inter)
            ...
            classification_loss = classification_loss_fn(logits_2d, labels_1d) # classification_loss만 해서 상대적으로 loss가 낮음.
        ...
        train_avg_loss = train_loss / (len(train_loader)) # 4배로 증강했기 때문에 이와 같이 설정함. (아직 증강 안함)
        ...
        wandb.log({"Train loss": train_avg_loss, "val_loss": val_loss, "val_auroc": val_auroc, "val_auprc": val_auprc})
        ...
        print(f"[Epoch {epoch}] Train Loss = {train_avg_loss: .4f}, Learning Rate = {current_lr:.6f}")
    ...
    wandb.log({"test_loss": test_loss, "test_auroc": test_auroc, "test_auprc": test_auprc})
